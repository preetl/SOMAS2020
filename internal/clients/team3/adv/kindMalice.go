...
type KindMalice struct {
	Id             shared.ClientID
	rulesToPropose map[string]rules.RuleMatrix
}

func (m *KindMalice) Initialise(id shared.ClientID) {
	m.Id = id
...
func (m *KindMalice) SetTaxationAmount(islandsResources map[shared.ClientID]shared.ResourcesReport) (shared.PresidentReturnContent, bool) {
	...
	for id, val := range islandsResources {
		if id != m.Id {
			...
			generatedMap[id] = val.ReportedAmount*0.2
		}
	}
...
func (m *KindMalice) ProposeRule(availableRules map[string]rules.RuleMatrix) (rules.RuleMatrix, bool) {
	return rules.RuleMatrix{}, false
}

func (m *KindMalice) GetRuleViolationSeverity() (map[string]shared.IIGOSanctionsScore, bool) {
	return map[string]shared.IIGOSanctionsScore{
		"check_taxation_rule": shared.IIGOSanctionsScore(50),
	}, true
}

func (m *KindMalice) GetPardonedIslands(currentSanctions map[int][]shared.Sanction) (map[int][]bool, bool) {
	pardons := make(map[int][]bool)
	for key, sanctionList := range currentSanctions {
		lst := make([]bool, len(sanctionList))
		pardons[key] = lst
		for index, _ := range sanctionList {
...
func (m *KindMalice) CallPresidentElection(monitoring shared.MonitorResult, turnsInPower int, allIslands []shared.ClientID) (shared.ElectionSettings, bool) {
...
func (m *KindMalice) DecideNextPresident(winner shared.ClientID) (shared.ClientID, bool) {
	return m.Id, true
}

func (m *KindMalice) CallJudgeElection(monitoring shared.MonitorResult, turnsInPower int, allIslands []shared.ClientID) (shared.ElectionSettings, bool) {
	return shared.ElectionSettings{
		VotingMethod:  shared.InstantRunoff,
		IslandsToVote: allIslands,
		HoldElection:  true,
...
func (m *KindMalice) DecideNextJudge(winner shared.ClientID) (shared.ClientID, bool) {
	return m.Id, true
}

func (m *KindMalice) CallSpeakerElection(monitoring shared.MonitorResult, turnsInPower int, allIslands []shared.ClientID) (shared.ElectionSettings, bool) {
	return shared.ElectionSettings{
		VotingMethod:  shared.InstantRunoff,
		IslandsToVote: allIslands,
		HoldElection:  true,
...
func (m *KindMalice) DecideNextSpeaker(winner shared.ClientID) (shared.ClientID, bool) {
	return m.Id, true
}

func (m *KindMalice) InspectHistory(iigoHistory []shared.Accountability, turnsAgo int) (map[shared.ClientID]shared.EvaluationReturn, bool, bool) {
